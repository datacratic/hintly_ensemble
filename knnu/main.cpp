...
/*
  build user-user similarity matrix, saved in ../data2/knnu-iif.txt
*/
...
/*
  load user-user similarity mastrix, saved in matrix w
*/
...
/*
  generate recommendation list for user "u" and saved in ret
*/
void predict(int u, vector< pair<int,float> > & ret){
     ...
     for(int v = 0; v < w[u].size() && v < 1000; ++v){
          ...
          for(int i = 0; i < ui[vv].size(); ++i){
               ...
               cand[ii] += w[u][v].second * w[u][v].second;
          }
     }
...
void predictAll(bool train){
     ...
     string file = "../ret2/results-knnu-iif2.txt";
     if(train) file += ".0";
     ofstream out(file.c_str());
